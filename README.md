...
- **Flow-aware edge ports** — which face an edge leaves is a 30° cone off the X-axis: near-level neighbours connect side-to-side, while a block one layer down but far off to the side connects **top-to-bottom**, so downward flow keeps reading as downward flow. `fromSide` / `toSide` (`"left" | "right" | "top" | "bottom"`) pin either end when geometry can't infer the intent.
- **Rich interactions**
