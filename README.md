...
- **Flow-aware edge ports** — which face an edge leaves is a 30° cone off the X-axis: near-level neighbours connect side-to-side, while a block one layer down but far off to the side connects **top-to-bottom**, so downward flow keeps reading as downward flow. Selection is **routing-aware** — if the cone's axis produces a curve that can't clear the blocks in its way, the edge is retried on the perpendicular axis and whichever actually routes is kept, so a face choice never costs you an invisible line. `fromSide` / `toSide` (`"left" | "right" | "top" | "bottom"`) pin either end when geometry can't infer the intent. Arrival/exit **angle** against the chosen face is corrected automatically too — an end grazing its face too shallowly is straightened onto the face's normal, yielding to obstacle clearance, so arrowheads read as entering the block instead of lying flat on its border.
- **Rich interactions**
